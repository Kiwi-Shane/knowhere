...
def load_url_upload_context(job_id: str, redis_service: Any) -> UrlUploadContext:
    ...
    job_metadata = (
        dict(raw_job_metadata) if isinstance(raw_job_metadata, dict) else {}
    )
    ...
    if not raw_s3_key or not job_metadata:
        ...
        if job_row:
            ...
            if not job_metadata and isinstance(job_row.job_metadata, dict):
                job_metadata = dict(job_row.job_metadata)
                if job_metadata:
                    metadata_service.save_metadata(job_id, job_metadata)
    ...
    return UrlUploadContext(
        s3_key=str(raw_s3_key),
